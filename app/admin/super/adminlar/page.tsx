...
export default function AdminsPage() {
  ...
  useEffect(() => setAdmins(getAdminAccounts()), []);
  ...
  function create(event: FormEvent) {
    ...
    try {
      ...
      setError("Ma’lumotlarni tekshiring: bu email oldin ishlatilgan bo‘lishi mumkin.");
    }
  }

  function toggle(item: AdminAccount) {
    try {
      setAdmins(setAdminActive(item.id, !item.active));
    } catch {
      setError("CEO o‘z Super Admin hisobini bloklay olmaydi.");
    }
  }
  ...
  return (
    <>
      ...
      {error && <p role="alert" className="mb-4 rounded-input bg-danger/10 p-3 text-xs text-danger">{error}</p>}
