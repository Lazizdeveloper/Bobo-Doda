...
import { getAdminData, handleUserAppeal } from "@/lib/api/admin";
...
export default function UserAppealsPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: data.appeals.length,
      pending: data.appeals.filter((a) => a.status === "pending").length,
      approved: data.appeals.filter((a) => a.status === "accepted").length,
      rejected: data.appeals.filter((a) => a.status === "rejected").length,
    };
  }, [data]);

  const filteredAppeals = useMemo(() => {
    if (!data) return [];
    return data.appeals.filter((a) => {
      if (statusFilter !== "all" && a.status !== statusFilter) return false;
      if (search.trim()) {
        const q = search.toLowerCase();
        return (
          a.userName.toLowerCase().includes(q) ||
          a.appealText.toLowerCase().includes(q) ||
  ...
  const paginatedAppeals = useMemo(() => {
    ...
    return filteredAppeals.slice(start, start + rowsPerPage);
  }, [filteredAppeals, currentPage, rowsPerPage]);
    ...
    <div className="space-y-6">
        ...
        {filteredAppeals.length ? (
          <>
            <Table
              ...
              renderMobileCard={(a) => (
                <div className="flex items-start justify-between gap-3 p-3 border border-line rounded-xl bg-card">
                  <div>
                    <p className="font-bold text-ink">{a.userName}</p>
                    <p className="text-2xs text-muted">Blok: {a.originalReason}</p>
                    <p className="text-xs text-ink mt-1 truncate max-w-xs">{a.appealText}</p>
                  </div>
                  <Button size="sm" variant="outline" onClick={() => setSelectedAppeal(a)}>
                    Ko‘rish
                  </Button>
                </div>
              )}
            ...
            <Pagination
              ...
              totalRows={filteredAppeals.length}
      ...
      {selectedAppeal && (
        <DangerousActionModal
          ...
          onConfirm={(reason) => {
            handleUserAppeal(selectedAppeal.id, actionDecision, reason);
            load();
