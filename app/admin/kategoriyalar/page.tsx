              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    toast(
                      cat.active
                        ? "Kategoriya o'chirildi — unda yangi ish e'lon qilib bo'lmaydi"
                        : "Kategoriya faollashtirildi"
                    );
                    load();
                  } catch (err) {
                    toast(adminErrorText(err), "error");
      ...
      {(editModalOpen || createModalOpen) && (
        ...
        >
          <div className="space-y-3.5 text-xs">
            {/* Yangi taksonomiya faqat backend bilan qo'shiladi: ilovadagi
                kategoriya ro'yxati (`ServiceCategory`) kompilyatsiya vaqtidagi
                tip bo'lib, har biriga o'ziga xos forma maydonlari bog'langan.
                Shu sababli slug erkin matn emas, mavjud ro'yxatdan tanlanadi. */}
            <p className="rounded-input border border-info/25 bg-info/10 p-2.5 text-2xs text-info-deep">
              {"Slug ilovadagi mavjud kategoriyalardan tanlanadi. Butunlay yangi kategoriya qo'shish backend va yangi deploy talab qiladi (har bir kategoriyaning o'z forma maydonlari bor)."}
            </p>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="font-semibold text-ink block mb-1">Slug (ilova kategoriyasi):</label>
                <Select
                  value={formSlug}
                  ...
                  options={[
                    { value: "", label: "Tanlang…" },
                    ...CATEGORIES.filter(
                      (c) => editModalOpen || !categories.some((x) => x.slug === c)
