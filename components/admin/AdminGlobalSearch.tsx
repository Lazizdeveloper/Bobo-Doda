...
import { adminGlobalSearch, type SearchResultItem } from "@/lib/api/admin";
...
export function AdminGlobalSearch() {
  ...
  /* Qidiruv ASYNC va DEBOUNCE bilan: backend'da bu har harfda ketadigan
     HTTP so'rov bo'lardi. Ilgari `adminGlobalSearch(query)` har render'da
     sinxron chaqirilardi — backend ulanganda bu ishlamay qolardi. */
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    adminGlobalSearch(debouncedQuery)
      .then((items) => {
        /* Kech kelgan javob yangisining ustiga yozmasligi kerak */
  ...
  const typeLabels: Record<string, { label: string; icon: AdminIconName }> = {
    user: { label: "Foydalanuvchi", icon: "users" },
    order: { label: "Shartnoma", icon: "orders" },
    kyc: { label: "KYC Hujjat", icon: "kyc" },
    dispute: { label: "Nizo / Arbitraj", icon: "disputes" },
    ticket: { label: "Yordam Chiptasi", icon: "support" },
    service: { label: "Xizmat", icon: "services" },
    job: { label: "Loyiha", icon: "jobs" },
  };
