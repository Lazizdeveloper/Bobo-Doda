...
For a single change (not a full production audit — see `production-readiness.md` for that). Run by the main coordinator per `.claude/review-matrix.md`.

## 1. Identify change category

Backend, frontend, database, financial, devops/infra, SRE/observability, CI/CD/release, or performance — per the review-matrix table. A change can span categories (e.g. a database migration touching the ledger is both Database and Financial) — apply every matching row's requirements, not just the first one that fits.
...
- `backend-engineer` implements an auth fix -> `security-engineer` review required, `qa-engineer` regression required.
